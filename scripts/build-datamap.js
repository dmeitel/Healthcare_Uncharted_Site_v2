...
const NODES = [
  ...
  { id: 'src-census-acs', kind: 'source', label: 'Census ACS 5-year', provenance: 'official',
    detail: 'American Community Survey demographics via the Census API (key required): median age, household size, age structure.' },
  { id: 'src-chr', kind: 'source', label: 'County Health Rankings', provenance: 'official',
    detail: 'UWPHI/RWJF analytic release: premature death (YPLL) and low birthweight, county grain with official state rows.' },
  ...
  { id: 'js-pull-acs', kind: 'script', label: 'pull/acs.js', stat: 'scripts/pull/acs.js',
    detail: 'Refreshable Census ACS demographics pull (needs a free API key). Dry-run by default.' },
  { id: 'js-pull-chr', kind: 'script', label: 'pull/chr.js', stat: 'scripts/pull/chr.js',
    detail: 'Refreshable County Health Rankings pull: the outcomes layer. Dry-run by default.' },
  ...
  { id: 'ds-county-data', kind: 'dataset', label: 'countyData.json', stat: 'src/assets/data/countyData.json',
    ...
    consumers: ['Population Health Map (eager)', 'Operators Map (eager)'] },
  ...
  { id: 'int-state-data', kind: 'intermediate', label: 'stateData.json', stat: 'src/_data/stateData.json',
    refresh: 'hand-maintained', provenance: 'official',
    detail: '7 lenses of state metrics, sourced per the metricsConfig ledger. EMBEDDED into the Population Health Map at build.',
    consumers: ['Population Health Map (embedded)', 'Learn/Rounds pages (embedded)', 'build-entities.js (read)'] },
  { id: 'int-metrics-config', kind: 'intermediate', label: 'metricsConfig.json', stat: 'src/_data/metricsConfig.json',
    ...
    consumers: ['Population Health Map (embedded)', 'build-entities.js (read)'] },
  { id: 'int-data-years', kind: 'intermediate', label: 'dataYears.json', stat: 'src/_data/dataYears.json',
    refresh: 'hand-maintained', provenance: 'editorial',
    detail: 'Data-vintage year stamps embedded at build.', consumers: ['Population Health Map (embedded)'] },
  ...
  { id: 'tool-operators-map', kind: 'tool', label: 'Operators Map', path: '/tools/operators-map/',
    detail: 'The facility map. Eager basemap + hospitals; every other layer lazy-loads on toggle.' },
  { id: 'tool-multi-lens', kind: 'tool', label: 'U.S. Population Health Map', path: '/tools/multi-lens-map/',
...
const EDGES = [
  ...
  { from: 'src-census-acs', to: 'js-pull-acs' },
  { from: 'src-chr', to: 'js-pull-chr' },
  ...
  { from: 'js-pull-acs', to: 'ds-county-data' },
  { from: 'js-pull-chr', to: 'ds-county-data' },
...
const PLAIN = {
  ...
  'src-census-acs':       'The Census Bureau\'s rolling survey of who lives where: median age, household size, age mix. The demographic feeder of the county metrics file.',
  'src-chr':              'County Health Rankings: university-built county health outcomes, most notably years of life lost before 75 and low birthweight. The outcomes feeder of the county metrics file.',
  ...
  'js-pull-acs':       'Fetches county demographics (median age, household size, age mix) from the Census API into countyData.json. Needs a free API key.',
  'js-pull-chr':       'Fetches county health outcomes (premature death, low birthweight) from County Health Rankings into countyData.json.',
  ...
  'int-state-data':        'State numbers for all seven lenses of the Population Health Map. Baked directly into that page when the site builds, so the map opens with zero data fetches.',
