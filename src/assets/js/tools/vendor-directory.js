/**
 * Healthcare Vendor Directory.
 *
 * Lifted out of an inline <script> on 2026-08-22, the first of the tools to move
 * (see docs/HU-BUILD-HARDENING-2026-08-22.md). Loaded as type="module", so it is
 * deferred, scoped, cacheable across pages, and finally visible to `npm run check`.
 *
...
/* ── typed DOM helpers ─────────────────────────────────────────────────────────
   checkJs sees getElementById as HTMLElement and querySelectorAll as Element, so
   .value, .dataset and .onclick all look unsafe. These three narrow once, here,
   instead of scattering casts through the file. Pure casts: no behaviour change. */

/** an input or select whose .value we read or write @param {string} id */
const byIdValue = id => /** @type {HTMLInputElement | HTMLSelectElement} */ (document.getElementById(id));

/** query as HTMLElements, so .dataset and .onclick resolve @param {ParentNode} root @param {string} sel */
const qsa = (root, sel) => /** @type {HTMLElement[]} */ (Array.from(root.querySelectorAll(sel)));

/** event target as an Element, for delegated .closest() lookups @param {Event} e @param {string} sel */
const hit = (e, sel) => /** @type {HTMLElement | null} */ (/** @type {Element} */ (e.target).closest(sel));
let vendors = [];
...
function buildSidebar() {
  ...
  nav.innerHTML = h;
  qsa(nav, '.vd-cat').forEach(b => b.onclick = () => setSector(b.dataset.sector));
}
...
function render() {
  const q = (byIdValue('searchInput').value || '').toLowerCase();
  ...
  qsa(document, '#vdNav .vd-cat').forEach(b => { const on = b.dataset.sector === activeSector; b.classList.toggle('active', on); b.setAttribute('aria-pressed', String(on)); });
  qsa(document, '#hqGrid .hqt').forEach(b => { const on = b.dataset.st === activeState; b.classList.toggle('active', on); b.setAttribute('aria-pressed', String(on)); });
  byIdValue('ownSel').value = activeOwn;
  byIdValue('sortSel').value = activeSort;
  byIdValue('shareSel').value = activeShare;
...
document.getElementById('popSectorList').addEventListener('click', e => { const t = hit(e, '[data-sector]'); if (!t) return; closePop(true); setSector(t.dataset.sector); });
...
document.getElementById('popViewsList').addEventListener('click', e => {
  ...
  activeSort = 'az'; byIdValue('searchInput').value = ''; openVendor = null;
  urlCtl.suspend(() => {
...
document.getElementById('vdApplied').addEventListener('click', e => {
  const t = hit(e, '[data-clear]'); if (!t) return;
...
function stateToParams() {
  ...
  const q = (byIdValue('searchInput').value || '').trim();
  if (q) p.set('q', q);
...
function applyURLState() {
  ...
  const qq = p.get('q'); byIdValue('searchInput').value = qq || '';
...
function openDetail(idx) {
  ...
  const x = /** @type {HTMLElement | null} */ (panel.querySelector('.icon-btn, button'));
...
function buildHQMap() {
  ...
  grid.innerHTML = h;
  qsa(grid, '.hqt').forEach(b => b.onclick = () => setState(b.dataset.st));
}
...
(function initCarousel(){
  ...
  qsa(dots, '.ms-dot').forEach(d => d.onclick = () => { showSlide(+d.dataset.i); msRestart(); });
...
/* ── event wiring ──────────────────────────────────────────────────────────────
   These were inline handlers on the markup until 2026-08-22. A module's functions
   are not globals, so they live here now. Inline handlers also require
   'unsafe-inline' in script-src, so moving them is part of clearing the CSP. */
document.getElementById('searchInput').addEventListener('input', () => { render(); queueSyncURL(); });
byIdValue('ownSel').addEventListener('change', e => setOwn(byIdValue('ownSel').value));
byIdValue('shareSel').addEventListener('change', e => setShare(byIdValue('shareSel').value));
byIdValue('sortSel').addEventListener('change', e => setSort(byIdValue('sortSel').value));
document.getElementById('dpClose').addEventListener('click', () => closeDetail());

/* one delegated listener owns every card, including the ones render() rebuilds.
   The cards were divs with onclick, so keyboard users could not reach them at all;
   role + tabindex ride the markup and Enter/Space are handled here. */
const vendorGrid = document.getElementById('vendorGrid');
vendorGrid.addEventListener('click', e => {
  const card = /** @type {HTMLElement | null} */ (hit(e, '.vendor-card[data-vendor-idx]'));
  if (card) openDetail(Number(card.dataset.vendorIdx));
});
vendorGrid.addEventListener('keydown', e => {
  if (e.key !== 'Enter' && e.key !== ' ') return;
  const card = /** @type {HTMLElement | null} */ (hit(e, '.vendor-card[data-vendor-idx]'));
  if (!card) return;
  e.preventDefault();
  openDetail(Number(card.dataset.vendorIdx));
});
