...
- Alaska/Hawaii insets: NOT possible on a geographic vector basemap (the AlbersUsa relocation was synthetic). Accepted trade; the future option is two corner inset mini-maps.
- Per-type marker ICONS: deferred — needs canvas-generated sprites (map.addImage); colors carry the distinction.
