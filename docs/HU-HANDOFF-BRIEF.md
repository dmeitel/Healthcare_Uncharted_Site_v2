...
- No new colors, typefaces, icon sets, or third-party scripts. Icons are Lucide (`<i data-lucide="name">`). External scripts are limited to a CSP allowlist (jsdelivr, unpkg, cdnjs, Google Fonts); anything else will not load.
...
- Floors that never move: 44 px touch targets on phones, 11 px functional text, the phone breakpoint is 699 px (never another), reduced motion honored, transform and opacity only in animation, `100dvh` never bare `100vh`.
