...
NEW FLAGGED (accent bg under light text, untouched): laws .lpv-inbar "1 hr" (12px/700, fill --jp-bg) inside .lpv-bar-teal, ~3.26:1 light theme; joins career-tree line-87 .hct-p-tabs button.on in that bucket.

RESIDUAL TAIL, re-verified by the 2026-08-22 site audit: src/index.html is DONE (hv-tab, hv-row .ty + hover, hv-door .da all carry teal-ink) and hu-global .section-tag / .nav-link.active / .hu-stat .v.hi / .hu-sr-row .why / .hu-bbar .bb-item.on are DONE. What is still raw --teal in light theme is all in hu-global SHARED CHROME and never had an override written: `.eyebrow` + `.section-eyebrow-v2` (11px, hits 404/about/vendor-directory + the three njk macros), `.tool-bar-title span` (15px/800, the accent word in every tool page h1), `.tool-attribution .ta-brand span` (11px/700) + `.tool-attribution a:hover` + `.tool-attribution--fixed .ta-toggle:hover` (every tool page, via components/tool-attribution.njk), `.nav-theme-btn:hover` (base.njk, every page), `.hu-fab:hover` / `.hu-fab.is-on` (icon, fails the 3:1 floor too), `.btn-ghost:hover` + `.btn-secondary` (both unused, dead rules), and the base `a { color:var(--brand-primary) }` fallback. Measured: teal is 1.72:1 on the light page bg (#EEF2F7) and 1.93:1 on #FFFFFF. teal-dk on the light PAGE bg is 3.08:1, not the 3.46 it gets on white; still clears the 3:1 non-text floor. Also assignment-compass and data-observatory were not re-checked in that pass. See [[defect-classes-and-origins]].
