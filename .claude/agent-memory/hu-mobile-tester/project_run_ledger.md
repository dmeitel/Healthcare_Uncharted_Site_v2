...
## Run 15 · 2026-09-20 · Device Assembly "Two walls" race (/secret-menu/device-assembly/), 360x740 + 699x900 isMobile/hasTouch, two contexts over the live Supabase relay — CLEAN, measurements only

Flow end to end at both widths: `T.host()` + `raceStart('t1')` → guest `T.join(code,'Sam')` → host `NET.seats.b==='Sam'` in 273 ms (360) / ~1 ms after the guest page settled (699) → host cannula + `netSync()` → guest `#ghostWall` opacity ".28", 2 children → guest recipe + `raceSubmit` → `race().winner==='Sam'` on both in ~800 ms; overlays read "You finished first." (guest) / "Sam finished first." (host). Zero real console errors (only my .11ty abort). `docSW===docCW` everywhere.

Every control in the fold and both overlays is 44 px or taller with (hover:none) true: summary 44, `#daName` 44, `#daHost`/`#daJoin` 144x44 (360) / 313.5x44 (699), `#daCode` 225.9x44 / 564.9x44, `#daJoinGo` 62.1x44, `#daLeave` 296x44 / 635x44, `#raceLeaveBtn` 148.3x44, `#raceBackBtn` 166.8x46. `#daJoinRow` is `display:none` + `hidden` until `#daJoin` is tapped (flex after, none again on the second tap; the `[hidden]` companion works here, unlike UG's `#ug-joinrow`). No text under 12 px anywhere (min 12 = `.da-k`/summary mono). `.da-code` 34 px mono, letter-spacing 11.9 px, one line, 129 px of text in a 296/635 px box. Overlay h2 "Table XXXX" 26 px one line. Fold width 296 at 360, 635 at 699, no internal overflow.

Harness lessons (in [[test-harness-quirks]]): the join overlay measured `#raceLeaveBtn` 143.8x42.7 at t=0 because `.da-modal` starts at `transform:scale(.97)` (220 ms); settled it is 148.3x44. And `scrollIntoView({block:'start'})` inside `#infoDock` slides the target under the `position:sticky; z-index:6` 40 px grab handle, which is NOT the fold's resting position (summary top 228.8/218.5 vs grab 68-108/81-121).

Screenshots: tmp/race/{360,699}-{1a-fold-closed,1b-fold-open,1c-fold-joinrow,1d-fold-natural-position,2-host-fold,2b-host-fold-seated,3a-guest-joining,3a2-guest-lobby-settled,3b-guest-board,4-guest-win,4-host-win}.png + {360,699}-results.json.

## Run 14 · 2026-09-20 · Uncharted General multiplayer lobby (/secret-menu/uncharted-general/), 360x740 isMobile + 699x900, two real contexts over the Supabase relay — measurements only

Flow works end to end: `hostTable()` → second context `joinTable(code,'Sam')` lands on the host roster in ~400 ms (chan.kind `internet`), `claimSeat('clinical')` and `netAct('start',{ceo:'mha'})` propagate, guest board banner `#ug-table` reads "Table XXXX · Sam · CCO · Clinical". Zero real console errors at both widths (only the .11ty abort + goatcounter localhost). Document scrollWidth never exceeded the viewport. No text under 12px (banner, `.ug-set-lbl`, `.ug-tablebar` sit at exactly 12px).

Measured facts worth keeping: (1) **`#ug-joinrow` renders while carrying `hidden`**: `.ug-load-row{display:flex}` (index.html:253) has no `[hidden]` companion, unlike `#ug-runhint[hidden]`, `.ug-deftip[hidden]`, `.ug-overlay[hidden]` — the CODE input + "Join ▸" (44px tall) are visible before anyone taps "Join a table", both widths. (2) **Start modal at 360 is horizontally scrollable by 35px** (`.ug-modal` scrollW 345 / clientW 310, overflow-x auto): the Ownership seg `button.lock "🔒 Academic"` right edge 370.1 and the Hospital seg "Custom" button right 367.7 poke past the 334px client box (both past the 360 viewport too). Clean at 699. (3) `.ug-expand` buttons (Host/Join/Close the table/Chart Room) are 38px tall at both widths; `.ug-seg` run-length buttons 35x36 at 360; `.ceo-arrow` 40x40; `.ceo-dot` 8x8; board `.ug-tut-toggle` 34px at 360 and 17px at 699. (4) Room code `.ug-roomcode` 34px mono, letter-spacing .35em, one line at both widths; the `.ug-set-lbl` "MULTIPLAYER · THE TABLE / Table XXXX" wraps to 2 lines each side at 360. (5) Guest lobby seats: 1 column 250px at 360 (grid `250px`), 2 columns 289.5px at 699; no horizontal scroll either width; at 360 the modal needs 182px of internal scroll to reach Observer + "Leave the table" (158.9x50). (6) The banner is single-column in flow at both widths (`.ug-side` no grid), 1576px down the page at 360, 1256 at 699 — not "on the left" on a phone.

Harness note: the guest page's `matchMedia('(hover:none)')` read FALSE after the board rendered (host page in an identical isMobile/hasTouch context read TRUE). Unexplained; sizes matched anyway. Screenshots: tmp/lobby/{360,699}-{1-start-menu,2-host-menu,3-guest-lobby,3b-guest-lobby-bottom,4-guest-board,4-guest-banner,4-host-board}.png.

## Run 13 · 2026-08-24 · NEW Learn module /learn/request-routing/ pre-publish, 360/390/430 portrait — ONE REAL DEFECT, everything else PASS

**The defect (unusable-grade for site navigation): `.rr-topnav` inherits hu-global.css:84's bare `nav{position:sticky;top:0;z-index:200;height:64px}` element selector.** Scroll UP mid-article: the site nav returns (nav-yield) and the stuck section-tab rail covers it 64px/64px at the same z; rail is later in DOM so it wins — brand/hamburger/search `elementFromPoint` to `rr-tnav`, unreachable everywhere below the bar's dock point (y≈2983). All three widths, no media query involved. Proof: tmp/mobile/rr-360-scrollup-navs.png. **The other two Learn pages with nested navs already solve this**: 4ps `.section-nav` (sticky top:64px z:5, index.html:177) and laws `.lp-tabs` (sticky top:64px z:20, :323). Fix = one line in the page style block: `.rr-topnav{ top:64px; z-index:150; }` (selectPage's `-76` scroll offset says sticky is intended, so don't static it). Same root gives the rail its 64px height + translucent bg — cosmetic, fine.

**Everything else passed at all three widths**: zero overflow on all 4 tabs (docSW===docCW and per-element walk; table + pocket-pre + both tab rails all contained in overflow-x:auto scrollers that scroll to their overflow and hit SELF); zero module controls under 44px (rr-tnav/rr-tab 46px, summaries 53, checkbox LABELS 44-64 and label-tap toggles, #rrDayClear 76x44, drill/router buttons all pass — only known site chrome: nav-brand 192x40, footer pills 28, newsletter 37, one inline 22px link); class tabs light #rrTree per class; drill correct/wrong/lock/progress all work; router walked to verdict on 3 distinct paths (enh-standard, fix via rq1=y skip, nw-project) with correct LEADS/SOR/approver every time, rv-open jumps + selects the class tab, rv-reset collapses; JPG export produced an 841,707-byte data URL at 360/390/430 with ZERO console errors (headless download fires and completes); Copy → real clipboard verified; details open on real touch; day checkboxes persist through reload (localStorage hu-informatics-day, date-keyed) and Clear now resets; deep link #reference fresh-load selects the tab (and every reload restores the hash tab); CLS 0.0010; images = 2 logo SVGs; NO 100vh; no hover-dependent content; console = only harness noise on every load and after every interaction.

**Imperfect (report-only): SVG annotation text is tiny on phones.** .sk/.sn mono labels render 7.2-8.0px at 360 (rrLife worst: 400-unit viewBox → scale 0.72), 8.0-8.8 at 390, 8.9-9.9 at 430. Figure annotations, not body copy; .rr-p body is 16px. Secondary copy sits 13.5-15px (.rr-fig-cap 13.5, .rr-tldr-p 14, .rr-drill-fb 14, .rr-lede 15) — the site's editorial scale, called out but not defect-ranked. H1 exactly 3 lines at 40px. No ellipsis truncation anywhere. rrPipe/rrLife rotated "FAILED · RETURN" captions getBBox past the viewBox = FALSE positive (rotate(90) transforms).

**Pass streaks:** first run for this page. Section tabs, class tabs, drill, router, export, copy, persistence, deep links: 1/1.
